...
| `_build.py` | Regenerates the ten pages. |
...
kicad-cli sch export svg --no-background-color -o /tmp/svg "../kicad/UTS Mini Mixing Desk - Compressor.kicad_sch"
#    then copy the seven sheets into img/ as connector.svg, input.svg, vca.svg,
...
This folder is called `docs/` because that is one of the two sources GitHub's simple Pages
UI offers (`/` and `/docs`), so either route works:

1. **Source: GitHub Actions** — `.github/workflows/pages.yml` publishes `docs/` on every push
   to `main`. This is what the repository is set up for.
2. **Source: Deploy from a branch → main → /docs** — no workflow needed. Delete
   `.github/workflows/pages.yml` if you take this route.

Nothing inside the site depends on the folder name. `.nojekyll` is present so Pages serves
the files as-is.
